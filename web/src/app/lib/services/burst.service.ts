...
import {Block, BlockChainStatus, HttpError, Settings} from '../model';
...
export class BurstService {
    ...
    // TODO: make returns more consistent...so return promises, or observables
    public getBlockchainStatus(): Observable<HttpEvent<BlockChainStatus | HttpError>> {
    ...
    public translateServerError(response) {
        ...
        switch (response.errorCode) {
            ...
            case 3:
                ...
                if (match && match[1]) {
                    let fieldNames = match[1].split(',');
                    let translatedFieldNames = [];

                    fieldNames.map((fieldName) => {
                        translatedFieldNames.push(this.getTranslatedFieldName(fieldName).toLowerCase());
                    });

                    let translatedFieldNamesJoined = translatedFieldNames.join(', ');
